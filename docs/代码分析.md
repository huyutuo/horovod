...
-----------------------------------------------------------------------------
 不管用什么通信后端，都是通过MPI进行negotiation?  rank 0被称为coordinator，它和其它rank进行coordination的过程由Request和Response(horovod/common/message.h中有详细注释)来表示。每个tensor一个Request，一个Response里可能会有多个tensor，发给所有的worker。每个Response对象对应一个collective operation操作。这些tensor放在fusion buffer里。
...
​    BackgroundThreadLoop：初始化controller(调用具体通信后端controller的DoInitialization方法)；设置参数的值，如fusion buffer大小与cycle周期、response cache大小等。
